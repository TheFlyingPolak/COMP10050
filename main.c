...
int main(){
	...
	if(ans == 1){	// Read players and slots from a file
		...
		playersFromUser(players, num_players);
		...
		enum slot_types slots[num_slots];

		/*  Call function to generate list of slots  */
		slotsFromUser(slots, num_slots);
	}
	else{
	...
	//populateBoard(players, slots, num_players, num_slots);
