...
/*This function receives input from the user to choose how many slots they wish for the game to have.*/
...
/*This function randomly assigns a player to a slot.*/
void populateBoard(struct player players[], struct slot slots[]){
	...
	while (i < num_players){
		slot = rand()%num_slots;	//Generate a random number.
		if(slots[slot].player == 0){	//If the random slot generate's player value is 0 then continue.
			players[i].position = slot;	// Set the players position value to be the random slot value.
			slots[slot].player = i+1;	// set the random slot value to be the player number.
			i++;	//Increment i by 1;
		}
	}
}
